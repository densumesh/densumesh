...
description: "How one noisy customer almost broke our system and why we built Broccoli to make queues fair for everyone"
pubDate: 2025-08-29
tags: ["rust", "performance", "distributed-systems", "queueing", "Broccoli"]
---

It was a late night at the Trieve office. The team was tired from a long day of shipping and ready to head home. Then our phones lit up. Customers were complaining all over our Slack and Discord about their documents failing to be indexed for multiple hours. It looked like a bomb had gone off in our system.

Our **biggest customer** had just dumped **millions of documents** into the ingestion pipeline, and suddenly other customers couldn't get their data indexed. Throwing more workers at the problem wasn’t helping. They were all busy chewing through this one massive job while everyone else’s data sat waiting in line.

# **The Anatomy of a Noisy Neighbor**

What we were seeing had a name: the **“noisy neighbor” problem**. In multi-tenant distributed systems, one greedy tenant can starve out the rest.
...
Now, imagine a new customer signs up with a **massive monorepo** containing tens of millions of files. The moment their job hits the pipeline, you can only watch in horror as your other customers' indexing requests grind to a halt. For the next several hours, your system is **held hostage by a single tenant**.

![image](https://hackmd.io/_uploads/SysLoIUYlg.png)  
> In a traditional **FIFO queue**, smaller jobs are stuck waiting behind one massive job.

This is the crux of the noisy neighbor problem, and it’s a fundamental flaw in systems built on a traditional **First-In, First-Out (FIFO)** messaging queue. FIFO only cares about **who got there first**, not **who needs serving most**.

# **Why Traditional Fixes Fall Short**

When you're dealing with noisy neighbors, there are a few common solutions that come to mind.

## 1. **Throttling Clients**
...
The reality was far from ideal. These customers would send massive bursts of work—say, uploading **hundreds of gigabytes** of data in an hour—followed by long periods of silence, leaving expensive worker instances **sitting completely idle**.
...
![image](https://hackmd.io/_uploads/Bkp7JwIKll.png)  
...
![image](https://hackmd.io/_uploads/BJ0zePUKll.png)
...
When I first open-sourced **Broccoli**, I had no idea whether or not people would use it, but the reception has been **insane**. Since then, the project has had **15,000+ downloads** and **400+ stars** on GitHub.
...
If you want to dig in deeper or try it out for yourself, check out [**Broccoli on GitHub**](https://github.com/densumesh/broccoli).
